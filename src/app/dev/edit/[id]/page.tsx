...
function EditGamePageContent() {
  ...
  useEffect(() => {
    if (gameData) {
      const isOwner = user?.uid === gameData.developerId;
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      // Preserve the original developerId, even if an admin is editing.
      const updatedGameData = {
        ...
        developerId: gameData.developerId, // Explicitly keep original developerId
      ...
      router.push(isAdmin ? '/admin/games' : '/dev/my-games');
  ...
  const canEdit = user && (gameData.developerId === user.uid || isAdmin);
  if (!canEdit) {
    return (
        <div className="flex min-h-screen flex-col items-center justify-center">
            ...
            <div className="flex items-center gap-4 mb-4">
                <Button asChild variant="outline" size="icon">
                    <Link href="/dev/my-games">
                        ...
                        <span className="sr-only">Back to My Games</span>
                    </Link>
                </Button>
                <div>
                    <CardTitle className="text-3xl font-headline">Edit Game</CardTitle>
                    <CardDescription>
